...
Topic: Issue #5 — parser must understand the new ai-factory plan markdown format (bold metadata, `- **Testing:** yes — ...`, `- **Status:** planning`, `### Phase N: Name` with `**Task N:** ...` bold tasks)
Goal: Adapt `packages/lazyaif/src/modules/plans-viewer/parser.ts` to parse the new plan format generated by ai-factory, so the TUI correctly shows titles, settings, phases, and tasks.
Constraints:
  - Current parser regexes: `PHASE_RE = /^###\s+Phase\s+\d+:\s*(.+)$/`, `TASK_RE = /^-\s+\[([ xX])\]\s+Task\s+(\d+):\s*(.+)$/`
  - Current settings regexes expect `- Testing: yes` (no `**`), `- Docs: yes/no` (no `warn-only`)
  - Current title regex: `/^#\s+Implementation Plan:\s*(.+)$/i`
  - New format uses: `# Plan: <title>` (not `# Implementation Plan:`), `**Branch:**`, `**Created:**`, `**Status:**`
  - New settings: `- **Testing:** yes — update existing tests + add new tests for \`entry\` region`, `- **Logging:** verbose — DEBUG logs...`, `- **Docs:** warn-only (no mandatory checkpoint)`
  - New tasks: `### Phase 1: Core format specification (docs first)` then `- [ ] **Task 1: Update openspec spec — add \`mim.entry\` requirement**` (bold task names, no `Task N:` prefix outside bold)
  - New sections: `## Settings`, `## Tasks`, `## Commit Plan`, `## Risks`, `## Notes`
  - Settings values can include trailing `—` descriptions; `Docs` can be `warn-only` not just `yes`/`no`
  - Phase regex matches `### Phase N: Name` — same pattern, OK
  - Task regex needs to handle `- [ ] **Task N: ...**` (bold-wrapped task names)
  - Need to extract `Status: planning` as a new field (optional)
  - Title regex needs to handle `# Plan: <title>` in addition to `# Implementation Plan: <title>`
Decisions:
  - Support BOTH old and new formats (backward compatible)
  - Title: try `# Implementation Plan: <title>` first, then `# Plan: <title>`
  - Branch/Created: support both `Branch: value` and `- **Branch:** value` patterns
  - Settings: support both `- Testing: yes` and `- **Testing:** yes — ...` (strip `—` and after)
  - Docs value: `yes` / `no` / `warn-only` — add `warn-only` to PlanSettings.docs type as string union
  - Status field: parse `- **Status:** planning` as optional string on Plan
  - Task regex: support `- [x] Task N: ...` AND `- [x] **Task N: ...**`
  - Task id extraction: `Task N:` or `**Task N:**` — extract N
Open questions:
  - Should `PlanSettings.docs` be `boolean | "warn-only"` or a string union? -> Use `boolean | "warn-only"` for compat, or widen to `"yes" | "no" | "warn-only"`
  - Existing tests assert `settings.docs` as boolean — need to map `warn-only` to false? Or change type?
  - Decision: keep `docs` as boolean for now (map `warn-only` → false internally), add `docsMode: string` optional field for the raw value
Success signals:
  - New format plans (like issue #5) parse correctly: title, branch, created, settings, phases, tasks
  - Old format plans still parse correctly (all 58 tests pass)
  - TUI shows correct title, status, progress for new-format plans
  - `Status: planning` shown in TUI metadata line
Next step: /aif-plan fast — update parser regexes + types + tests + TUI meta line
...
### 2026-07-03 00:00 — Issue #5: parser must handle new ai-factory plan format (bold metadata)
What changed:
  - Issue #5 reveals that ai-factory generates a NEW markdown plan format that the lazyaif parser doesn't understand
  - Old format: `# Implementation Plan: <title>`, `Branch: none`, `- Testing: yes`, `- [x] Task N: ...`
  - New format: `# Plan: <title>`, `- **Branch:** ...`, `- **Testing:** yes — description`, `- **Status:** planning`, `- [x] **Task N: ...**`
  - Parser regexes in `packages/lazyaif/src/modules/plans-viewer/parser.ts` match only old format
  - `PlanSettings.docs` is `boolean` but new format uses `warn-only` value
  - New `Status: planning` field not represented in `Plan` type
Key notes:
  - `PHASE_RE` already works for both formats (`### Phase N: Name`)
  - `TASK_RE` needs to handle `**Task N:` bold prefix
  - Title regex needs `# Plan:` in addition to `# Implementation Plan:`
  - Settings regexes need `**` and trailing `—` description stripping
  - `Branch`/`Created` lines use `- **Branch:**` in new format vs bare `Branch:` in old
Links (paths):
  - packages/lazyaif/src/modules/plans-viewer/parser.ts:24-29 — regexes
  - packages/lazyaif/src/modules/plans-viewer/types.ts:8-12 — PlanSettings
  - packages/lazyaif/src/views/plans-viewer/tui-view.ts:185 — meta line
  - packages/lazyaif/tests/modules/plans-viewer/parser.test.ts — existing tests
  - packages/lazyaif/tests/fixtures/*.md — existing fixtures (old format)
